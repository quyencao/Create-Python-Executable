...
class DownloadHelper(object):
    ...
    def process(self):
        ...
        try:
            request = requests.get(self.url, timeout=120, stream=True)
...
class ProcessHelper(multiprocessing.Process):
    def __init__(self, run_command, install_command=None, id=None, require_install_packages=True):
        super(ProcessHelper, self).__init__()
        self.CODE_DIR = '/twain/code'
        self.MODEL_DIR = '/twain/model'
        self.id = id
        self.run_command = run_command
        self.install_command = install_command
        self.env = self.get_env()
        self.cwd = self.CODE_DIR
        self.require_install_packages = require_install_packages
    
    def get_env(self):
        env = os.environ.copy()
        env['MODEL_DIRECTORY'] = self.MODEL_DIR
        env['SOURCE_DIRECTORY'] = self.CODE_DIR
    ...
    def run(self):
        ...
        if self.require_install_packages:
            ...
            install_proc = psutil.Popen(self.install_command, env=self.env, stdout=PIPE, stderr=PIPE)
...
class Consumer(object):
    ...
    def __init__(self, amqp_parameters, queue, routing_key):
        ...
        self._parameters = amqp_parameters
    ...
    def on_message(self, _unused_channel, basic_deliver, properties, body):
        ...
        if command == 'model':
            ...
            download = DownloadHelper(url, self.MODEL_TMP_DIR, self.MODEL_DIR)
            download.process()
        elif command == 'source':
            url = payload["data"]["url"]
            download_manager = DownloadHelper(url, self.CODE_TMP_DIR, self.CODE_DIR)
...
class ReconnectingConsumer(object):
    ...
    def __init__(self, queue, routing_key, host='localhost', port=5672, virtual_host='/', username='guest', password='guest'):
        self._reconnect_delay = 0
        credentials = pika.PlainCredentials(username, password)
        self._parameters = pika.ConnectionParameters(host=host, port=port, virtual_host=virtual_host, credentials=credentials)
        self._queue = queue
        self._routing_key = routing_key
        self._consumer = Consumer(self._parameters, self._queue, self._routing_key)
...
def main():
    ...
    with open('/twain/config/config.json', 'r') as json_file:
    ...
    consumer = ReconnectingConsumer(queue, routing_key, host=host)
    consumer.run()
